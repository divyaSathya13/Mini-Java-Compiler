...
Block:'{'S  '}';
S:Assignment S |T_BREAK';' {if(ss==0){ss=1;$$.addr=newLabel(&ln);tt=$$.addr;$$.code = (char*)malloc(sizeof(char)*100);sprintf($$.code,"goto %s\n",$$.addr); fprintf(fp,"%s",$$.code);}else{$$.code = (char*)malloc(sizeof(char)*100);sprintf($$.code,"goto %s\n",tt);fprintf(fp,"%s",$$.code);}} S |T_CONTINUE';' S|T_IF '('Expression')' {$$.tr=newLabel(&ln);$$.addr=newLabel(&ln);pp=$$.addr;$$.code = (char*)malloc(sizeof(char)*100);sprintf($$.code,"if %s goto %s\nelse goto LABEL%d\n%s:",$3.addr,$$.tr,count,$$.tr);count+=1;fprintf(fp,"%s",$$.code);} Block {$$.tr=newLabel(&ln);qq=$$.tr;$$.code = (char*)malloc(sizeof(char)*100);sprintf($$.code,"goto %s\n",$$.tr);fprintf(fp,"%s",$$.code);} WW {$$.code = (char*)malloc(sizeof(char)*100);sprintf($$.code,"%s:",qq);fprintf(fp,"%s",$$.code);}S| T_RETURN Expression';' S|T_SWITCH'('Expression')' {rr=$3.addr;} '{'SwitchBlock'}' {if(ss==1){$$.addr=tt;$$.code = (char*)malloc(sizeof(char)*100);sprintf($$.code,"%s:",$$.addr); fprintf(fp,"%s",$$.code);}} S |Variable_declaration S|Array_declaration';' S|Array_initialisation';' S | T_PRINT'('Expression')' ';' S |
H';' |;
WW:T_ELSE {count-=1;$$.code = (char*)malloc(sizeof(char)*100);sprintf($$.code,"LABEL%d:",count);fprintf(fp,"%s",$$.code);} Block|{count-=1;$$.code = (char*)malloc(sizeof(char)*100);sprintf($$.code,"%s:\n",pp);fprintf(fp,"%s",$$.code);};
H:Expr W | W Expr;
W:T_INC | T_DEC;
